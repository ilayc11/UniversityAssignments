...
Space: .asciiz " "
newLine: .asciiz "\n"
CommaAndSpace: .asciiz ", "
BackshlashZeroByte: .asciiz "\0"
...
addCustomerFunc:
	...
	#sw $v0,4($a0)
    	...
    	# bytes to go further to reach the 104th byte. 
    	# For example, if the name 'Yossiel' , $t1 <- 100 - 7 = 93
    	
    		    subi $a0, $a0, 1
    		    subi $s2, $s2, 1
    		    lb $t0, BackshlashZeroByte
    		    sb $t0, ($a0)
        	    li $t1, 100
        	    sub $t1, $t1, $s2
...
displayCustomerFunc:
	...
	LookForIDLoop:
	beq $a0,$t1,CustomerNotInDatabase
...
deleteRecordFunc:
	#looks for the id in memory, if theres a match we run on each memory and make it zero
	...
	#delete record from stack
	LookForIDInStack:#if we reach here already know the id exist, so no need to jump to didntfindid part 
	lw $t2,($a0)
	lw $t2,($t2)
	beq $t2,$t0,FoundIDAddressInStack
	addi $a0,$a0,4
	j LookForIDInStack
	
	FoundIDAddressInStack: #store zero in end of stack, and move the address instead of address to be deleted
	lw $t1,($sp)
	sw $zero,($sp)
	addi $sp,$sp,4
	sw $t1,($a0)
